...
type SortMode = 'manual' | 'name' | 'stock' | 'price'
...
export default function InventoryTab({ userId }: { userId: string }) {
  ...
  const moveProduct = async (fullList: Product[], index: number, dir: -1 | 1) => {
    const newIndex = index + dir
    if (newIndex < 0 || newIndex >= fullList.length) return
    const a = fullList[index]
    const b = fullList[newIndex]
  ...
  // گروه‌بندی کالاها بر اساس دسته‌بندی برای نمایش کشویی
  const groups: { key: string; label: string; items: Product[] }[] = [
    ...categories.map((c) => ({ key: c.id, label: c.name, items: products.filter((p) => p.category_id === c.id) })),
    { key: 'none', label: 'بدون دسته', items: products.filter((p) => !p.category_id) },
  ].filter((g) => g.items.length > 0 || g.key !== 'none')

  const sortItems = (items: Product[]) => {
    const arr = [...items]
    if (sortMode === 'name') arr.sort((a, b) => a.name.localeCompare(b.name, 'fa'))
    else if (sortMode === 'stock') arr.sort((a, b) => a.stock_qty - b.stock_qty)
    else if (sortMode === 'price') arr.sort((a, b) => a.sell_price - b.sell_price)
    // manual: همان ترتیب sort_order که از قبل مرتب آمده
    return arr
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <button type="button" onClick={() => setCategoriesOpen(!categoriesOpen)} className="w-full flex items-center justify-between mb-4">
          <h2 className="text-white font-bold">دسته‌بندی کالاها ({categories.length})</h2>
          <span className="text-slate-400">{categoriesOpen ? '▲' : '▼'}</span>
        </button>
        {categoriesOpen && (
          <>
            ...
            <div className="space-y-2">
              {categories.map((c, i) => (
                <div key={c.id} className="flex items-center justify-between bg-slate-800/50 rounded-lg p-3 flex-wrap gap-2">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <h2 className="text-white font-bold">کالاها ({products.length})</h2>
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-400">ترتیب نمایش:</label>
            <select value={sortMode} onChange={(e) => setSortMode(e.target.value as SortMode)} className="bg-slate-800 text-white rounded-lg p-2 text-xs">
              <option value="manual">دستی (پیش‌فرض)</option>
              <option value="name">الفبا</option>
              <option value="stock">میزان موجودی</option>
              <option value="price">قیمت فروش</option>
            </select>
          </div>
        </div>

        <form onSubmit={submitProduct} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        ...
        <div className="space-y-3">
          {groups.map((g) => {
            ...
            return (
              <div key={g.key} className="border border-slate-800 rounded-xl overflow-hidden">
                <button type="button" onClick={() => toggleGroup(g.key)} className="w-full flex items-center justify-between bg-slate-800/60 px-4 py-3">
                  <span className="text-white text-sm font-medium">{g.label} ({items.length})</span>
                            ...
                            <tr key={p.id} className="border-b border-slate-800/50">
                              ...
                              <td className="p-2">
                                <div className="flex gap-1">
                                  {sortMode === 'manual' && (
                                    <>
                                      <button onClick={() => moveProduct(products, globalIndex, -1)} className="text-slate-400 hover:text-white px-1">▲</button>
                                      <button onClick={() => moveProduct(products, globalIndex, 1)} className="text-slate-400 hover:text-white px-1">▼</button>
                                    </>
                                  )}
          ...
          {products.length === 0 && <p className="text-slate-500 text-sm text-center py-4">هنوز کالایی ثبت نشده است.</p>}
        </div>
      </section>
    </div>
  )
